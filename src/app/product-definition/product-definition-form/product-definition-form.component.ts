...
export class ProductDefinitionFormComponent implements OnInit {
  ...
  constructor(private router: Router, private productDefinitionService: ProductDefinitionService, private optionService: OptionService,private datePipe: DatePipe) { 
    ...
    if (this.productDefinitionId != null && this.productDefinitionId > 0){
      this.productDefinition$ = this.productDefinitionService.getProductDefinitionById(this.productDefinitionId).subscribe(result => {
        console.log(result);
        const formattedStartDate = this.datePipe.transform(result.startDate,'yyyy-MM-dd') ?? '';
        const formattedEndDate = this.datePipe.transform(result.endDate,'yyyy-MM-dd') ?? '';
        const formattedSubscriptionClosing = this.datePipe.transform(result.subscriptionClosing,'yyyy-MM-dd') ?? '';
        const formattedSubscriptionOpening = this.datePipe.transform(result.subscriptionOpening,'yyyy-MM-dd') ?? '';
  ...
  onSubmit(): void{
    ...
    if (this.isEdit) {
      ...
      this.putProductDefinition$ = this.productDefinitionService.putProductDefinition(this.productDefinitionId, this.productDefinitionForm.getRawValue()).subscribe(result => {
        //all went well
