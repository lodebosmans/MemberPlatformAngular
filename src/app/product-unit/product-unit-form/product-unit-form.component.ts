...
export class ProductUnitFormComponent implements OnInit {
  ...
  constructor(private router: Router, private productUnitService: ProductUnitService, 
    private optionService: OptionService, private addressService:AddressService, 
    private productDefinitionService: ProductDefinitionService,private datePipe: DatePipe) {
      this.isAdd = this.router.getCurrentNavigation()?.extras.state?.['mode'] === 'add';
      this.isEdit = this.router.getCurrentNavigation()?.extras.state?.['mode'] === 'edit';
      this.productUnitId = +this.router.getCurrentNavigation()?.extras.state?.['id'];
      console.log('in constructor: ' + this.productUnitId)

      if (this.productUnitId != null && this.productUnitId > 0){
        this.productUnit$ = this.productUnitService.getProductUnitById(this.productUnitId).subscribe(result => {
          console.log(result);
  ...
  ngOnInit(): void {
    this.productUnitForm.get('endTimeActual')?.valueChanges.subscribe(value => {
      if (value) {
        ...
        this.productUnitForm.get('endTimeActual')?.setValue(formattedTime, { emitEvent: false });
      }
    });
    this.productUnitForm.get('startTimeActual')?.valueChanges.subscribe(value => {
      if (value) {
        const timeValue = value.split(':');
        const hours = timeValue[0];
        const minutes = timeValue[1];
        const seconds = '00';
        const formattedTime = `${hours}:${minutes}:${seconds}`;
        this.productUnitForm.get('startTimeActual')?.setValue(formattedTime, { emitEvent: false });
      }
    });
    this.productUnitForm.get('endTimeScheduled')?.valueChanges.subscribe(value => {
      if (value) {
        const timeValue = value.split(':');
        const hours = timeValue[0];
        const minutes = timeValue[1];
        const seconds = '00';
        const formattedTime = `${hours}:${minutes}:${seconds}`;
        this.productUnitForm.get('endTimeScheduled')?.setValue(formattedTime, { emitEvent: false });
      }
    });
    this.productUnitForm.get('startTimeScheduled')?.valueChanges.subscribe(value => {
      if (value) {
        const timeValue = value.split(':');
        const hours = timeValue[0];
        const minutes = timeValue[1];
        const seconds = '00';
        const formattedTime = `${hours}:${minutes}:${seconds}`;
        this.productUnitForm.get('startTimeScheduled')?.setValue(formattedTime, { emitEvent: false });
      }
    });
    if(this.isEdit){
  ...
  getProductUnitById() {
    this.productUnit$ = this.productUnitService.getProductUnitById(this.productUnitId).subscribe(result => this.productUnit = result);
  }
  ...
  onSubmit(): void{
    ...
    if (this.isEdit) {
      ...
      this.putProductUnit$ = this.productUnitService.putProductUnit(this.productUnitId, this.productUnitForm.getRawValue()).subscribe(result => {
        //all went well
