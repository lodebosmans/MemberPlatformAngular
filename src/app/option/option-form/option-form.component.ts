...
export class OptionFormComponent implements OnInit {
  ...
  constructor(private router: Router, private optionService: OptionService, private optionTypeService: OptionTypeService) {
    ...
    if (this.optionId != null && this.optionId > 0){
      this.option$ = this.optionService.getOptionById(this.optionId).subscribe(result => {
        console.log(result);
    ...
    this.optionType$ = this.optionTypeService.getOptionTypes().subscribe(result => {
  ...
  getOptionById() {
    this.option$ = this.optionService.getOptionById(this.optionId).subscribe(result => this.option = result);
  }
  ...
  onSubmit(): void {
    ...
    if (this.isEdit) {
      ...
      this.putOption$ = this.optionService.putOption(this.optionId, this.optionForm.getRawValue()).subscribe(result => {
        //all went well
    ...
    if(this.isAdd) {
      console.log(" in Add")
      console.log(this.optionId)
      this.postOption$ = this.optionService.postOption(this.optionForm.getRawValue()).subscribe(result => {
        this.router.navigateByUrl("/option");
      },
      error => {
        this.errorMessage = error.message;
